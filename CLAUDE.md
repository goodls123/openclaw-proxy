...
├── version.py                 # 版本信息
├── requirements.txt           # 依赖（paramiko, pyinstaller）
...
│
├── models/                    # 数据模型
│   ├── config.py              # 配置模型（SSHConfig, BrowserConfig等）
...
│
├── resources/                 # 资源文件
│   └── images/                # 图片资源（favicon.ico, openclaw.png）
│
├── tests/                     # 测试
...
- **格式**: INI格式，通过 `ConfigRepository` 读写
- **配置结构**: 见 `models/config.py` (SSHConfig, BrowserConfig, KeyGenConfig, AppConfig)
